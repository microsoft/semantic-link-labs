...
def warm_direct_lake_cache_perspective(dataset: str, perspective: str, add_dependencies: Optional[bool] = False, workspace: Optional[str] = None):
    ...
    if not any(r['Mode'] == 'DirectLake' for i, r in dfP.iterrows()):
        print(f"{red_dot} The '{dataset}' semantic model in the '{workspace}' workspace is not in Direct Lake mode. This function is specifically for semantic models in Direct Lake mode.")
        return
    
    dfPersp = fabric.list_perspectives(dataset = dataset, workspace = workspace)
    dfPersp['DAX Object Name'] = format_dax_object_name(dfPersp['Table Name'], dfPersp['Object Name'])
    dfPersp_filt = dfPersp[dfPersp['Perspective Name'] == perspective]

    if len(dfPersp_filt) == 0:
        print(f"{red_dot} The '{perspective} perspective does not exist or contains no objects within the '{dataset}' semantic model in the '{workspace}' workspace.")
        return
    ...
    if add_dependencies:
        ...
        md['Referenced Full Object'] = format_dax_object_name(md['Referenced Table'], md['Referenced Object'])
        dfPersp_m = dfPersp_filt[(dfPersp_filt['Object Type'] == 'Measure')]
        md_filt = md[(md['Object Name'].isin(dfPersp_m['Object Name'].values)) & (md['Referenced Object Type'] == 'Column')]
        ...
        dfH_filt = dfH[dfH['Hierarchy Object'].isin(dfPersp_h['DAX Object Name'].values)]
        ...
        merged_list = np.concatenate([column_values, measureDep, hierarchyDep, fromObjects, toObjects])
    ...
    print(f"{green_dot} The following columns have been put into memory:")

    new_column_order = ['Table Name', 'Column Name', 'DAX Object Name']
    df = df.reindex(columns=new_column_order)
    df = df[['Table Name', 'Column Name']].sort_values(by=['Table Name', 'Column Name'], ascending=True)
...
def warm_direct_lake_cache_isresident(dataset: str, workspace: Optional[str] = None):
    ...
    dfP = fabric.list_partitions(dataset = dataset, workspace = workspace)
    if not any(r['Mode'] == 'DirectLake' for i, r in dfP.iterrows()):
        print(f"The '{dataset}' semantic model in the '{workspace}' workspace is not in Direct Lake mode. This function is specifically for semantic models in Direct Lake mode.")
        return
    
    # Identify columns which are currently in memory (Is Resident = True)
    dfC = fabric.list_columns(dataset = dataset, workspace = workspace, extended = True)
    dfC['DAX Object Name'] = format_dax_object_name(dfC['Table Name'], dfC['Column Name'])
    dfC_filtered = dfC[dfC['Is Resident']]

    if len(dfC_filtered) == 0:
        print(f"{yellow_dot} At present, no columns are in memory in the '{dataset}' semantic model in the '{workspace}' workspace.")
        return
    ...
    print(f"{green_dot} The following columns have been put into memory. Temperature indicates the column temperature prior to the semantic model refresh.")

    return dfC_filtered[['Table Name', 'Column Name', 'Is Resident', 'Temperature']].sort_values(by=['Table Name', 'Column Name'], ascending=True)
