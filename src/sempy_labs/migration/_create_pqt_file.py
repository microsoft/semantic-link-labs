...
from sempy_labs._dataflows import get_dataflow_definition
...
@log
def create_pqt_file_from_dataflow(
    workspace: str | UUID,
    dataflow: str | UUID,
    file_name: str = "PowerQueryTemplate",
):
    """
    Dynamically generates a `Power Query Template <https://learn.microsoft.com/power-query/power-query-template>`_ file based on the dataflow definition. The .pqt file is
    saved within the Files section of your lakehouse.
    ...
    if not lakehouse_attached():
        raise ValueError(
            f"{icons.red_dot} In order to run the 'create_pqt_file_from_dataflow' function, a lakehouse must be attached to the notebook. Please attach a lakehouse to this notebook."
        )

    (workspace_name, workspace_id) = resolve_workspace_name_and_id(workspace)
    (dataflow_name, dataflow_id) = resolve_dataflow_id(dataflow, workspace_id)

    dataflow_definition = get_dataflow_definition(workspace_id, dataflow_id)
    ...
    for entity in dataflow_definition["entities"]:
        table_name = entity["name"]
        for char in icons.special_characters:
            table_name = table_name.replace(char, "")
        table_map[table_name] = entity["expression"]
    ...
    def create_pqt(table_map: dict, expr_map: dict, file_name: str):
        ...
        for t_name, query in table_map.items():
            queryMetadata.append(QueryMetadata(t_name, None, None, None, True, False))
        for e_name, kind_expr in expr_map.items():
            e_kind = kind_expr[0]
            if e_kind == "M":
                queryMetadata.append(
        ...
        rootObject = RootObject("en-US", "2.132.328.0", queryMetadata)  # "2.126.453.0"
        ...
        print(
            f"{icons.green_dot} '{file_name}.pqt' has been created based on the '{dataflow_name}' dataflow in the '{workspace_name}' workspace within the Files section of your lakehouse."
        )

    create_pqt(table_map, expr_map, file_name=file_name)
