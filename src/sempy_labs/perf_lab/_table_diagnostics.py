...
def get_storage_table_column_segments(
    ...
    if refresh_type is not None:
        _refresh_test_models(
            test_suite = test_cycle_definitions,
            refresh_type = refresh_type,
        )

    # Initialize an empty DataFrame
    results_df = None
    for row in test_cycle_definitions.to_df().dropDuplicates(
        ...
        try:
            (target_workspace_name, target_workspace_id) = (
                resolve_workspace_name_and_id(workspace=target_workspace)
            )
        ...
        try:
            (target_dataset_name, target_dataset_id) = resolve_dataset_name_and_id(
                dataset=target_dataset, workspace=target_workspace_id
            )
        except (WorkspaceNotFoundException, ValueError):
        ...
        if dfD_filt.empty:
            print(
                f"{icons.red_dot} Unable to find test semantic model '{target_dataset_name}'. Please review your test definitions and make sure all test semantic models are provisioned."
            )
            continue
        else:
