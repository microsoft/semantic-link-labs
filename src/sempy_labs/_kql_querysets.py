...
def list_kql_querysets(workspace: Optional[str | UUID] = None) -> pd.DataFrame:
    ...
    responses = _base_api(request=f"v1/workspaces/{workspace_id}/kqlQuerysets", uses_pagination=True)
...
def create_kql_queryset(
    ...
    _base_api(request=f"v1/workspaces/{workspace_id}/kqlQuerysets", method="post", payload=payload, status_codes=[201, 202], lro_return_status_code=True)
...
def delete_kql_queryset(name: str, workspace: Optional[str | UUID] = None):
    ...
    _base_api(request=f"/v1/workspaces/{workspace_id}/kqlQuerysets/{kql_database_id}", method="delete")
    print(
