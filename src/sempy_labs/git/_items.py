...
def connect_workspace_to_github(
    ...
    connection_id: UUID,
    source: Literal[
        "ConfiguredConnection", "Automatic", "None"
    ...
    source : typing.Literal["ConfiguredConnection", "Automatic", "None"], default="ConfiguredConnection"
...
@log
def get_status(workspace: Optional[str | UUID] = None) -> pd.DataFrame:
    """
...
@log
def get_connection(workspace: Optional[str | UUID] = None) -> pd.DataFrame:
    """
    Obtains the Git status of items in the workspace, that can be committed to Git.
...
@log
def initialize_connection(workspace: Optional[str | UUID] = None) -> str:
    """
