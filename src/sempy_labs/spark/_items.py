...
def list_custom_pools(workspace: Optional[str | UUID] = None) -> pd.DataFrame:
    ...
    response = _base_api(
        request=f"/v1/workspaces/{workspace_id}/spark/pools", client="fabric_sp"
    )
...
def delete_custom_pool(pool_name: str, workspace: Optional[str | UUID] = None):
    ...
    _base_api(
        request=f"/v1/workspaces/{workspace_id}/spark/pools/{pool_id}",
        method="delete",
        client="fabric_sp",
    )
    print(
...
def get_spark_settings(
    ...
    response = _base_api(
        request=f"/v1/workspaces/{workspace_id}/spark/settings", client="fabric_sp"
    )
