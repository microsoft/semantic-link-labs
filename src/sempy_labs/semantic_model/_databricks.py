...
def _generate_semantic_model_from_metric_view(
    ...
    for source in sources:
        ...
        mirror_connection_id = mirror_catalog.get('properties', {}).get('databricksWorkspaceConnectionId')
        
        df_filt = df[df['Connection Id'] == mirror_connection_id]
        if df_filt.empty:
            raise ValueError()
        mirror_connection_workspace = df_filt['Connection Path'].iloc[0].rstrip('/')
        if mirror_connection_workspace != databricks_workspace.rstrip('/'):
            raise ValueError()
        if catalog != mirror_catalog_name:
            raise ValueError(
                f"{icons.warning} The catalog name '{catalog}' in the model map does not match the catalog name '{mirror_catalog_name}' of the mirrored Azure Databricks Catalog item."
            )
    ...
    model_id = create_blank_semantic_model(dataset=name, workspace=workspace, overwrite=False)
    with connect_semantic_model(
        dataset=model_id, workspace=workspace, readonly=False
    ) as tom:
