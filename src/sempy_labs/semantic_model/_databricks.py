...
def _add_table_to_model_map(
    ...
    source_item_id, source_workspace_id = source.get("itemId"), source.get(
        "workspaceId"
    )
...
def _create_model_map(
    ...
    for join in joins:
        ...
        model_map = _add_table_to_model_map(
            source_object=join_source,
            model_map=model_map,
            is_source=False,
            source_name=join_name,
            source_list=source_list,
        )
            ...
            t["columns"].append(
                {
                    ...
                    "columnReferences": list(
                        set(
                            [
                                f"{src_name}.{fixed_source_column}",
                                fixed_source_column,
                                fixed_column_name,
                            ]
                        )
    ...
    for col in cols:
        if not col.get("identified"):
            print(
                f"{icons.warning} Skipping the '{col.get('name')}' column as calculated columns are not supported."
            )
            ...
            if not tbl_name:
                print(
                    f"{icons.warning} Could not determine table for column '{column_name}'"
                )
                continue
