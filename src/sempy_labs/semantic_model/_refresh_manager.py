...
.slls-rm-header-action { color:var(--ui-accent);background:var(--ui-accent-soft);border-color:transparent; }
...
function render({ model, el }) {
    ...
    const scrollContainers=()=>{const nodes=[],seen=new Set();let node=root;while(node){if(node instanceof Element&&node.scrollHeight>node.clientHeight&&!seen.has(node)){nodes.push(node);seen.add(node);}node=node.parentNode||(node.host||null);}const page=document.scrollingElement;if(page&&!seen.has(page))nodes.push(page);return nodes;};
    const keepPosition=()=>{savedPosition=scrollContainers().map(node=>({node,left:node.scrollLeft,top:node.scrollTop}));};
    const restorePosition=()=>{if(!savedPosition)return;const positions=savedPosition;savedPosition=null,restore=()=>positions.forEach(({node,left,top})=>node.scrollTo(left,top));restore();requestAnimationFrame(()=>{restore();requestAnimationFrame(restore);});};
