...
function render({ model, el }) {
    ...
    function picker(){if(!s.pickWs)s.pickWs=model.get("workspace_id")||"";const workspaces=model.get("workspaces")||[],datasets=(model.get("datasets")||{})[s.pickWs];const datasetPlaceholder=!s.pickWs?"Select a workspace first...":datasets===undefined?"Loading...":!datasets.length?"No semantic models":"Search semantic models...";return`<div class="slls-rm-picker-wrap"><div class="slls-rm-picker"><div class="slls-rm-picker-top"><div><div class="slls-rm-picker-title">Choose a semantic model</div><div class="slls-rm-picker-sub">Pick a workspace and semantic model to manage its refreshes.</div></div><button class="slls-rm-iconbtn" data-picker-reload title="Reload workspaces and semantic models" aria-label="Reload workspaces and semantic models">${I.refresh}</button></div><div class="slls-rm-picker-grid"><div class="slls-rm-field"><label>Workspace</label>${combo("workspace",workspaces,s.pickWs,"Search workspaces...")}</div><div class="slls-rm-field"><label>Semantic model</label>${combo("dataset",datasets||[],s.pickDs,datasetPlaceholder,!s.pickWs||datasets===undefined)}</div></div><div class="slls-rm-picker-actions">${model.get("connected")?`<button class="slls-rm-btn" data-picker-cancel>Cancel</button>`:""}<button class="slls-rm-btn primary" data-picker-connect ${!s.pickDs?"disabled":""}>Connect</button></div></div></div>`;}
            ...
            tables.append(
                {
                    ...
                    "kind": (
                        ...
                        if table.CalculationGroup is not None
                            ...
                            "calculated_table"
                            if any(
                                str(getattr(partition, "SourceType", "")).split(".")[-1]
                                == "Calculated"
                                for partition in table.Partitions
                            )
                            else "table"
            ...
            with fabric.create_trace_connection(
                ...
                with trace_connection.create_trace(icons.refresh_event_schema) as trace:
                    ...
                    while True:
                        ...
                        widget.gantt_events = _get_refresh_trace_events(
                            trace.get_trace_logs(), partition_map
                        )
                        if status in {
                            "Completed",
                            "Failed",
