...
.slls-rm-iconbtn { width:34px;height:34px;padding:0;border:1px solid var(--ui-border-strong);border-radius:8px;background:var(--ui-bg);color:var(--ui-text);display:inline-flex;align-items:center;justify-content:center;cursor:pointer; }
...
.slls-rm-picker-wrap { display:flex;align-items:flex-start;justify-content:stretch; }
.slls-rm-picker { width:100%;border:1px solid var(--ui-border);border-radius:14px;background:var(--ui-surface);padding:16px; }
.slls-rm-picker-top { display:flex;align-items:center;justify-content:space-between;gap:16px;margin-bottom:14px; }
.slls-rm-picker-head { min-width:0; }
.slls-rm-picker-title { font-size:14px;font-weight:600; }
.slls-rm-picker-sub { margin-top:3px;color:var(--ui-text-secondary);font-size:12.5px; }
.slls-rm-picker-grid { display:flex;align-items:flex-end;gap:10px;flex-wrap:wrap; }
.slls-rm-picker-grid .slls-rm-field { flex:1 1 240px;min-width:0; }
.slls-rm-picker-grid .slls-rm-field label { padding-left:4px;color:var(--ui-text-tertiary);font-size:11px;font-weight:600;letter-spacing:.04em;text-transform:uppercase; }
.slls-rm-picker-grid .slls-ss-btn { border-radius:999px;padding:7px 12px 7px 15px;background:var(--ui-surface);font-size:13.5px; }
.slls-rm-picker-grid .slls-ss-panel { z-index:90; }
.slls-rm-picker-actions { display:flex;align-items:center;justify-content:flex-end;gap:10px;margin-top:14px; }
...
.slls-rm-gantt-search { position:relative;margin-bottom:12px; }
.slls-rm-gantt-search span { position:absolute;left:10px;top:50%;transform:translateY(-50%);display:flex;color:var(--ui-text-tertiary);pointer-events:none; }
.slls-rm-gantt-search svg { width:14px;height:14px; }
.slls-rm-gantt-search .slls-rm-input { min-height:34px;padding-left:32px;font-size:12px; }
...
.slls-rm-gantt-rows { display:flex;flex-direction:column;gap:5px;max-height:50vh;overflow-y:auto;scrollbar-gutter:stable;padding-right:4px; }
.slls-rm-gantt-row { display:grid;grid-template-columns:minmax(120px,190px) minmax(0,1fr);align-items:center;gap:12px; }
.slls-rm-gantt-row.filtered { display:none; }
...
@media (max-width:800px) { .slls-rm-grid { grid-template-columns:1fr; } .slls-rm-wide { grid-column:auto; } .slls-rm-tree { height:300px; } .slls-rm-schedule-grid { grid-template-columns:1fr; } .slls-rm-summary { grid-template-columns:repeat(2,minmax(0,1fr)); } }
@media (max-width:640px) { .slls-rm-picker-grid { align-items:stretch;flex-direction:column; } }
""".replace("__LIGHT_THEME__", _UI_LIGHT_VARS).replace("__DARK_THEME__", _UI_DARK_VARS)
_WIDGET_CSS += "\n" + _UI_SEARCH_SELECT_CSS
_WIDGET_CSS += _ui_scoped_header_css(".slls-rm")
...
function render({ model, el }) {
    ...
    const s={full:true,tables:new Set(),parts:new Set(),expanded:new Set(),search:"",ganttSearch:"",history:false,schedule:false,type:"Full",commit:"Transactional",parallel:10,retry:0,policy:false,visualize:false,detail:false,draft:null,pickWs:"",pickDs:"",pickerOpen:false};
    ...
    let savedPosition=null,positionVersion=0;
    ...
    const keepPosition=()=>{positionVersion++;savedPosition=scrollContainers().map(({node,key})=>({node,key,left:node.scrollLeft,top:node.scrollTop}));};
    const restorePosition=()=>{if(!savedPosition)return;const positions=savedPosition,version=positionVersion;savedPosition=null;const restore=()=>{if(version!==positionVersion)return;positions.forEach(({node,key,left,top})=>{const target=key==="body"?root.querySelector(".slls-rm-body"):node;if(target)target.scrollTo(left,top);});};restore();requestAnimationFrame(()=>{restore();requestAnimationFrame(restore);});[50,150,300].forEach(delay=>setTimeout(restore,delay));};
    const preservePanelPosition=()=>{keepPosition();const positions=savedPosition;requestAnimationFrame(()=>{if(savedPosition===positions)restorePosition();});};
    root.addEventListener("mousedown",event=>{const panel=event.target.closest(".slls-rm-panel"),panelHead=event.target.closest(".slls-rm-panel-head"),action=event.target.closest("[data-reload-history],[data-reload-schedule],[data-cancel-history]");if(panel)preservePanelPosition();if(panelHead||action)event.preventDefault();},true);
    root.addEventListener("click",event=>{const panelHead=event.target.closest(".slls-rm-panel-head"),reload=event.target.closest("[data-reload-history],[data-reload-schedule]");if(!panelHead||reload)return;const toggle=panelHead.querySelector("[data-panel]");if(!toggle)return;event.preventDefault();event.stopPropagation();const open=toggle.dataset.panel==="history"?(s.history=!s.history):(s.schedule=!s.schedule);panelHead.closest(".slls-rm-panel").classList.toggle("open",open);if(toggle.dataset.panel==="history"&&open&&!model.get("history_loaded")){keepPosition();dispatch("load_history");}if(toggle.dataset.panel==="schedule"&&open&&!model.get("schedule_loaded")){keepPosition();dispatch("load_schedule");}},true);
    ...
    ["mousedown","click","change"].forEach(type=>root.addEventListener(type,event=>{if(event.target.closest(".slls-rm-panel"))event.stopPropagation();}));
    ...
    function picker(){if(!s.pickWs)s.pickWs=model.get("workspace_id")||"";const connected=!!model.get("connected"),busy=!!model.get("busy");return`<div class="slls-rm-picker-wrap"><div class="slls-rm-picker"><div class="slls-rm-picker-top"><div class="slls-rm-picker-head"><div class="slls-rm-picker-title">Connect to a semantic model</div><div class="slls-rm-picker-sub">Select a workspace and semantic model to begin.</div></div><button class="sl-reload-btn${busy?" sl-spinning":""}" data-picker-reload type="button" title="Reload workspaces and semantic models" aria-label="Reload workspaces and semantic models" ${busy?"disabled":""}>${I.refresh}</button></div><div class="slls-rm-picker-grid"><div class="slls-rm-field"><label>Workspace</label><div data-picker-ws></div></div><div class="slls-rm-field"><label>Semantic model</label><div data-picker-ds></div></div></div><div class="slls-rm-picker-actions">${connected?`<button class="slls-rm-btn" data-picker-close>Cancel</button>`:""}<button class="slls-rm-btn primary slls-rm-picker-connect" data-picker-connect ${!s.pickDs||busy?"disabled":""}>Connect</button></div></div></div>`;}
    function header(){const connected=!!model.get("connected"),picking=!connected||s.pickerOpen,subtitle=connected?`${esc(model.get("dataset_name"))} &middot; ${esc(model.get("workspace_name"))}`:"Choose a workspace and semantic model",fullscreenLabel=fsMode?"Exit full screen":"Full screen";return`<header class="slls-rm-header"><span class="slls-rm-mark">${I.refresh}</span><div class="slls-rm-heading"><div class="slls-rm-title-row"><h1 class="slls-rm-title">Refresh Manager</h1>${connected&&!picking?`<button class="sl-change-btn" data-change-model title="Change semantic model / workspace" aria-label="Change semantic model / workspace" ${model.get("busy")?"disabled":""}>${I.swap}</button>`:""}</div><div class="slls-rm-subtitle">${subtitle}</div></div><button class="sl-theme-btn" data-fullscreen title="${fullscreenLabel}" aria-label="${fullscreenLabel}">${fsMode?I.fullscreenExit:I.fullscreen}</button><button class="sl-theme-btn" data-theme title="Toggle theme">${model.get("dark_mode")?I.sun:I.moon}</button></header>`;}
    ...
    const updateGantt=()=>{const current=root.querySelector("[data-gantt-search]"),focused=document.activeElement===current,start=focused?current.selectionStart:null,end=focused?current.selectionEnd:null,scrollTop=root.querySelector(".slls-rm-gantt-rows")?.scrollTop||0;updateLive("[data-live-gantt]",gantt);applyGanttFilter();const rows=root.querySelector(".slls-rm-gantt-rows");if(rows)rows.scrollTop=scrollTop;if(focused){const input=root.querySelector("[data-gantt-search]");if(input){input.focus();input.setSelectionRange(start,end);}}};
    root.addEventListener("input",event=>{if(event.target.matches("[data-gantt-search]"))applyGanttFilter();});
    ["objects","objects_loading","history","history_loaded","history_loading","schedule_loaded","schedule_loading","busy","detail","dark_mode","connected","workspaces","datasets"].forEach(n=>model.on(`change:${n}`,draw));model.on("change:refresh_status",()=>updateLive("[data-live-status]",status));model.on("change:gantt_events",updateGantt);model.on("change:schedule",()=>{s.draft=null;draw();});model.on("change:connect_done",()=>{s.pickerOpen=false;s.pickDs="";s.tables.clear();s.parts.clear();s.expanded.clear();s.search="";s.ganttSearch="";s.history=false;s.schedule=false;s.draft=null;s.visualize=false;draw();});draw();if(model.get("connected"))dispatch("load_objects");
}
