...
.slls-rm-picker-reload { display:inline-flex;align-items:center;justify-content:center;flex:0 0 auto;width:32px;height:32px;padding:0;border:1px solid var(--ui-border-strong);border-radius:50%;background:var(--ui-surface);color:var(--ui-text);cursor:pointer; }
.slls-rm-picker-reload:hover:not(:disabled) { border-color:var(--ui-text-tertiary);background:var(--ui-bg-secondary); }
.slls-rm-picker-reload:disabled { opacity:.5;cursor:not-allowed; }
.slls-rm-picker-reload svg { width:14px;height:14px; }
.slls-rm-picker-reload.slls-rm-loading svg { animation:slls-rm-spin 900ms linear infinite; }
...
function render({ model, el }) {
    ...
    function picker(){if(!s.pickWs)s.pickWs=model.get("workspace_id")||"";const connected=!!model.get("connected"),busy=!!model.get("busy");return`<div class="slls-rm-picker-wrap"><div class="slls-rm-picker"><div class="slls-rm-picker-top"><div class="slls-rm-picker-head"><div class="slls-rm-picker-title">Connect to a semantic model</div><div class="slls-rm-picker-sub">Select a workspace and semantic model to begin.</div></div><button class="slls-rm-picker-reload${busy?" slls-rm-loading":""}" data-picker-reload type="button" title="Reload workspaces and semantic models" aria-label="Reload workspaces and semantic models" ${busy?"disabled":""}>${I.refresh}</button></div><div class="slls-rm-picker-grid"><div class="slls-rm-field"><label>Workspace</label><div data-picker-ws></div></div><div class="slls-rm-field"><label>Semantic model</label><div data-picker-ds></div></div></div><div class="slls-rm-picker-actions">${connected?`<button class="slls-rm-btn" data-picker-close>Cancel</button>`:""}<button class="slls-rm-btn primary slls-rm-picker-connect" data-picker-connect ${!s.pickDs||busy?"disabled":""}>Connect</button></div></div></div>`;}
