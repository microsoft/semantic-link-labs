...
def delete_kql_database(name: str, workspace: Optional[str] = None):
    ...
    dfK_filt = dfK[dfK['KQL Database Name'] == name]

    if len(dfK_filt) == 0:
        raise ValueError(f"{icons.red_dot} The '{name}' KQL database does not exist within the '{workspace}' workspace.")
    kql_database_id = dfK_filt['KQL Database Id'].iloc[0]
